...
def predict_and_show(features: dict, header: str):
    ...
    with c1:
        st.markdown(
            f"<div style='padding:18px;border-radius:10px;background:{COLOR[pred]};color:white;"
            f"text-align:center'><h2 style='margin:0'>Credit Score: {pred}</h2>"
            f"<p style='margin:0'>Confidence: {prob_map[pred]*100:.1f}%</p></div>",
            unsafe_allow_html=True)
    with c2:
        order = ["Poor", "Standard", "Good"]
        fig = go.Figure(go.Bar(x=[prob_map[c] * 100 for c in order], y=order, orientation="h",
                               marker_color=[COLOR[c] for c in order],
                               text=[f"{prob_map[c]*100:.1f}%" for c in order], textposition="auto"))
        fig.update_layout(height=220, margin=dict(l=0, r=0, t=10, b=0), xaxis_title="Probability (%)",
